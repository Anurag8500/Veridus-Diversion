...
export default function UniversitySignIn() {
    return (
        <AuthLayout
            ...
            description="Sign in to access your institutional dashboard and manage academic credentials."
        >
            <form className="space-y-5" onSubmit={(e) => e.preventDefault()}>
                <div className="space-y-4">
                    <div className="space-y-2">
                        <label className="text-xs font-medium text-text-secondary uppercase tracking-wider">Official Institution Email</label>
                ...
                <button className="w-full mt-6 bg-brand text-background-base rounded-xl py-3.5 px-4 font-medium hover:bg-brand-hover hover:scale-[1.02] active:scale-95 transition-all shadow-[0_0_20px_rgba(255,255,255,0.1)] flex items-center justify-center gap-2">
                    Login to University Dashboard <ArrowRight className="w-4 h-4" />
                </button>
            </form>
            ...
            <div className="mt-8 pt-6 border-t border-border-subtle flex flex-col gap-4 text-center">
                <p className="text-sm text-text-secondary">
                    New institution?{" "}
                    <Link href="/signup/university" className="text-text-primary hover:text-brand font-medium transition-colors">
